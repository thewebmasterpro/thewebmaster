...
const categoryLabels: Record<string, { label: string; icon: typeof Search }> = {
  technical: { label: "Analyse Technique", icon: Settings },
  onpage: { label: "Analyse On-Page", icon: FileSearch },
  performance: { label: "Performance", icon: Zap },
  offpage: { label: "Analyse Off-Page", icon: Share2 },
};
...
function MetaPreview({ meta, url }: { meta: SeoAuditResult["meta"]; url: string }) {
  const displayUrl = (() => {
    try {
      const u = new URL(url);
  ...
  return (
    <div className="p-6 rounded-2xl border border-border/50 bg-card/50">
      <div className="flex items-center gap-2 mb-4">
        ...
        <h3 className="text-lg font-semibold">Apercu Google (SERP)</h3>
      </div>
      <div className="bg-white dark:bg-zinc-900 rounded-xl p-4 border border-border/30 max-w-2xl">
        ...
        <p className="text-lg text-blue-600 dark:text-blue-400 font-medium mb-1 line-clamp-1 hover:underline cursor-pointer">
          {meta.title || "Titre manquant"}
        </p>
        <p className="text-sm text-zinc-600 dark:text-zinc-400 line-clamp-2">
          {meta.description || "Aucune meta description definie. Google generera un extrait automatiquement."}
        </p>
      </div>
    </div>
  );
...
function HeadingStructure({ headings }: { headings: { tag: string; text: string }[] }) {
  ...
  return (
    <div className="p-6 rounded-2xl border border-border/50 bg-card/50">
      <div className="flex items-center gap-2 mb-4">
        ...
        <h3 className="text-lg font-semibold">Structure des titres</h3>
        <span className="text-xs text-muted-foreground">({headings.length} titres)</span>
      </div>
...
function QuickStats({
  ...
  responseTime: number;
  pageSize: number;
  links: SeoAuditResult["links"];
  images: SeoAuditResult["images"];
}) {
  const stats = [
    {
      icon: Zap,
...
function generatePDF(result: SeoAuditResult) {
  ...
  doc.text("RAPPORT D'AUDIT SEO", marginL, 18);
  ...
  const summaryItems = [
    ...
    ["Temps de reponse", `${result.responseTime}ms`],
    ["Taille HTML", `${Math.round(result.pageSize / 1024)} KB`],
  ...
  doc.setTextColor(17, 24, 39);
  doc.setFont("helvetica", "bold");
  doc.text("2. Meta donnees", marginL, y);
  y += 8;
  drawLine(y);
  y += 6;

  const metaItems = [
    ["Title", result.meta.title || "MANQUANT"],
    ["Description", result.meta.description || "MANQUANTE"],
    ["Canonical", result.meta.canonical || "ABSENTE"],
    ["Langue", result.meta.language || "NON DEFINIE"],
    ["OG Image", result.meta.ogImage ? "Presente" : "ABSENTE"],
  ];
  ...
  doc.setTextColor(17, 24, 39);
  doc.setFont("helvetica", "bold");
  doc.text("3. Statistiques", marginL, y);
  y += 8;
  drawLine(y);
  y += 6;

  const statsItems = [
    ["Liens internes", `${result.links.internal}`],
    ["Liens externes", `${result.links.external}`],
    ["Images totales", `${result.images.total}`],
    ["Images sans alt", `${result.images.withoutAlt}`],
  ];
  ...
  const catLabelsMap: Record<string, string> = {
    ...
    onpage: "Analyse On-Page",
    performance: "Performance",
    offpage: "Analyse Off-Page",
  };
  ...
  for (let i = 1; i <= totalPages; i++) {
    ...
    doc.setFont("helvetica", "normal");
    doc.text(
      `Rapport SEO - ${hostname} - ${date} - Page ${i}/${totalPages}`,
      marginL,
      H - 10
    );
    doc.text("The Webmaster - SEO Audit Tool", W - marginR - 50, H - 10);
  }

  doc.save(`seo-audit-${hostname}-${new Date().toISOString().split("T")[0]}.pdf`);
}

function SeoReport({ result }: { result: SeoAuditResult }) {
  const [copied, setCopied] = useState(false);
  const { quickWins, warnings, improvements } = generateSeoReportText(result);

  const reportText = `RAPPORT D'AUDIT SEO
${"=".repeat(50)}
Site : ${result.url}
...
---
Genere par TheWebmaster SEO Audit Tool
`;
      ...
      <div className="space-y-6">
        ...
        {quickWins.length === 0 && warnings.length === 0 && improvements.length === 0 && (
          <div className="text-center py-6">
            ...
            <p className="text-sm text-muted-foreground">Excellent ! Aucun probleme majeur detecte.</p>
          </div>
        )}
      </div>
    </div>
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="text-center mb-10">
          ...
          <h1 className="text-3xl md:text-4xl font-bold mb-3">
            Audit SEO <span className="text-primary">Complet</span>
          </h1>
          <p className="text-muted-foreground max-w-xl mx-auto">
            Analysez en profondeur le SEO d&apos;un site web : technique, on-page,
            performance, off-page et recommandations priorisees.
          </p>
        </div>
        ...
        {status === "loading" && (
          <div className="text-center py-20">
            ...
            <p className="text-muted-foreground animate-pulse">
              Analyse SEO en cours... Verification technique, on-page, performance et off-page.
            </p>
          </div>
        )}
        ...
        {result && (
          <div className="space-y-8">
            ...
            <div className="p-8 rounded-2xl border border-border/50 bg-card/50 backdrop-blur-sm">
              <div className="flex flex-col md:flex-row items-center gap-8">
                ...
                <div className="flex-1 text-center md:text-left">
                  <h2 className="text-xl font-bold mb-1">Resultat de l&apos;audit SEO</h2>
            ...
            {/* SERP Preview */}
            <MetaPreview meta={result.meta} url={result.url} />
            ...
            {/* Heading Structure */}
            <HeadingStructure headings={result.headings} />
            ...
            <div className="p-4 rounded-xl border border-border/30 bg-muted/20 text-center">
              <p className="text-xs text-muted-foreground">
                Cet outil analyse les informations publiquement accessibles (HTML, headers HTTP, fichiers standards).
                Il ne remplace pas un audit SEO professionnel complet avec acces a Google Search Console et Analytics.
                Analyse le {new Date(result.timestamp).toLocaleString("fr-BE")} — Temps de reponse : {result.responseTime}ms
              </p>
            </div>
          </div>
        )}
